...
硬性版式安全线:任何生成结果都不得出现文字越过边框、文字贴住装饰框线、图标/圆点/图片压住文字、多个小图标挤成一团、数据点标签彼此覆盖。遇到定位矩阵、气泡图、图标阵列、圆形节点、封面年份框等高风险页面时,必须优先保证安全边距和可读性;必要时移动标签、减少节点、换页或放大安全间距,不要只靠缩小字体勉强塞进去。
