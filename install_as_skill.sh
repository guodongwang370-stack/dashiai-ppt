...
##############################################################################
# 栋哥PPT -- Claude Code / Codex Skill 安装脚本
#
...
resolve_skill_dir() {
    case "$1" in
        claude)
            echo "$HOME/.claude/skills/dongge-ppt"
            ;;
        codex)
            echo "${CODEX_HOME:-$HOME/.codex}/skills/dongge-ppt"
            ;;
        openclaw)
            echo "$HOME/skills/dongge-ppt"
            ;;
    esac
}
...
main() {
    ...
    if [ "$INSTALL_TARGET" = "codex" ]; then
        print_info "  3. 在 Codex 里直接说：'帮我用栋哥PPT生成一份 5 页 PPT'"
        print_info "     如果当前 Codex 自带原生出图能力，可直接走原生路径，不必配置 OPENAI_API_KEY"
    else
        print_info "  3. 直接对 $AGENT_LABEL 说：'帮我用栋哥PPT生成一份 5 页 PPT'"
    fi
    echo ""
    print_info "冒烟测试（可选）："
